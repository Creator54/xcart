...
def place_order(
    ...
    if not cart_items:
        logger.warning(f"User {current_user.email} tried to place order with empty cart")
    ...
    logger.info(f"User {current_user.email} placed order #{order.id} with {items_count} items, total: ${total_amount:.2f}")
    return order
